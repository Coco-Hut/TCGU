...
def get_dataset(name, normalize_features=False, transform=None):
    ...
    if name in ['ogbn-arxiv', 'ogbn-products', 'ogbn-papers100M']:
        ...
        dataset = Amazon(root, name=name, transform=T.NormalizeFeatures())
...
def split_train_test_val(num_nodes,train_ratio=0.6,test_ratio=0.2):
    ...
    idx_test=idx[train_len:train_len+test_len]
    idx_val=idx[train_len+test_len:]
    
    return idx_train,idx_val,idx_test

def cosine_similarity(h1: torch.Tensor, h2: torch.Tensor):
    h1 = F.normalize(h1)
    h2 = F.normalize(h2)
    return h1 @ h2.t()
